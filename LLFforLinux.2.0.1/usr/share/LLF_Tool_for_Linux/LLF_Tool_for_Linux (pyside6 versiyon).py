...
class FormatWorker(QThread):
    progress_signal = Signal(dict)  # Yüzde, hız, sektör gibi veriler için
    finished_signal = Signal(bool, str) # Başarı durumu ve mesaj
    ...
    def run(self):
        ...
        try:
            ...
            # --synchronous: Veriyi fiziksel olarak yazana kadar 'bitti' demez.
...
class LLFToolSkeleton(QWidget):
    def __init__(self):
        ...
        # Set global font to Liberation Sans
    ...
    def handle_continue_button(self):
        ...
        device_path = self.device_table.item(selected_row, 0).data(Qt.UserRole)
    ...
    def update_device_details(self, device_path):
        ...
        try:
            # -i yerine -a kullanarak daha geniş bilgi alalım
    ...
    def show_about_dialog(self):
        ...
        about_text = (
            ...
            "on Debian-based systems, modeled after HDD GURU's famous software. This program is "
            "NOT official software from HDD GURU; it is a free, open-source alternative.<br><br>"
        ...
        msg.setIconPixmap(QIcon(self.icon_path).pixmap(64, 64)) # Menü içinde 64x64 boyutunda görünür
    ...
    def is_system_device(self, device_path):
        try:
            # lsblk ile cihazın mount pointlerini (bağlantı noktalarını) kontrol et
    ...
    def refresh_device_list(self):
        ...
        try:
            ...
            for row, dev in enumerate(devices):
                ...
                if is_sys:
                    model_item.setForeground(Qt.red) # Sistem diskini kırmızı yap
    ...
    # Cihaz yolunu (path) gizli veri olarak ilk sütuna saklayalım
                self.device_table.item(row, 0).setData(Qt.UserRole, path)
    ...
    def refresh_smart_data(self, device_path):
        ...
        try:
            ...
            # NVMe ve ATA ayrımı (Senin smartdiskdoctor.py mantığın)
    ...
    def create_device_selection_page(self):
        ...
        link_label = QLabel("<a href='https://www.github.com/shampuan'>www.github.com/shampuan</a>")
        ...
        self.device_info_line.setAlignment(Qt.AlignCenter)
        layout.addWidget(self.device_info_line)

        # Cihaz Listesi Tablosu
        self.device_table = QTableWidget(5, 6) # Örnek 5 satır
        ...
        about_link_selection.setCursor(Qt.PointingHandCursor)
        about_link_selection.linkActivated.connect(self.show_about_dialog)
        layout.addWidget(about_link_selection)
        warning_note.setStyleSheet("color: red; font-weight:")
        ...
        continue_btn.setFixedSize(120, 30)
    ...
    def create_operation_page(self):
        ...
        self.top_device_label = QLabel("[0] ST500DM002-1BD142  KC45  [500.1 GB]")
        self.top_device_label.setFont(QFont("Liberation Sans", 10, QFont.Bold))
        
        support_link = QLabel("Visit: <a href='https://www.github.com/shampuan'>github.com/shampuan</a>")
        ...
        self.speed_label.setFont(QFont("Liberation Sans", 12, QFont.Bold))
        ...
        self.smart_table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        ...
        self.smart_table.setEditTriggers(QTableWidget.NoEditTriggers)
        ...
        about_link.linkActivated.connect(self.show_about_dialog) # Tıklanınca fonksiyonu çağırır
    ...
    def handle_get_smart_click(self):
        # İlk sayfadaki tabloda hangi satır seçiliyse onu buluyoruz
        selected_row = self.device_table.currentRow()
        if selected_row == -1:
            QMessageBox.warning(self, "Warning", "Please select a device from the list first.")
            return
...
if __name__ == "__main__":
    ...
    # Eğer root değilsek, pkexec ile kendini yeniden başlat
    if os.geteuid() != 0:
    ...
    # Sistem temasını (GTK/Dark) root oturumuna taşımak için argüman ekliyoruz
    sys_args = sys.argv + ['-platformtheme', 'gtk3']
    app = QApplication(sys_args)
    
    # Fusion stilini koruyarak sistem renk paletine uymasını sağlıyoruz
